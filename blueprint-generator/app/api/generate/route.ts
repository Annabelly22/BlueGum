...
export async function GET() {
  ...
  // ── Gumroad: Bearer header ──
  try {
    const token = process.env.GUMROAD_ACCESS_TOKEN
    if (!token) throw new Error('not set')
    const { default: axios } = await import('axios')
    const res = await axios.get('https://api.gumroad.com/v2/user', {
      headers: { Authorization: `Bearer ${token}` }
    })
    results.gumroad_bearer = 'OK — ' + res.data?.user?.email
  } catch (e: any) {
    results.gumroad_bearer = 'FAIL: ' + (e?.response?.status || e.message)
  }

  // ── Gumroad: query param (legacy) ──
  try {
    const token = process.env.GUMROAD_ACCESS_TOKEN
    if (!token) throw new Error('not set')
    const { default: axios } = await import('axios')
    ...
    results.gumroad_queryparam = 'FAIL: ' + (e?.response?.status || e.message)
  }
  ...
  try {
    ...
    if (!token) throw new Error('not set')
    const { default: axios } = await import('axios')
    const res = await axios.get(`https://api.gumroad.com/v2/products?access_token=${token}`)
    results.gumroad_products = res.data?.success
      ? `OK — ${res.data.products.length} products found`
      : 'FAIL: ' + JSON.stringify(res.data)
  } catch (e: any) {
    results.gumroad_products = 'FAIL: ' + (e?.response?.status || e.message)
  }
  ...
  try {
    ...
    results.supabase = bucket
      ? `OK — blueprints bucket found, public=${bucket.public}`
      : 'FAIL — blueprints bucket not found. Buckets: ' + data?.map((b: any) => b.name).join(', ')
  } catch (e: any) {
  ...
  results.env_check = {
    ...
    GUMROAD_ACCESS_TOKEN: process.env.GUMROAD_ACCESS_TOKEN
      ? process.env.GUMROAD_ACCESS_TOKEN.slice(0, 12) + '...'
      : 'NOT SET',
